...
pip install modelscope==1.16.1 # 阿里达摩院的「魔搭」模型库
pip install transformers==4.43.1 # huggingface 核心库
pip install accelerate==0.32.1  # 加速训练 / 推理
pip install peft==0.11.1  # 高效微调模型
pip install datasets==2.20.0  # 数据集处理工具
```
...
# Hugging Face 的自动加载器，会根据模型配置文件自动识别架构类型（如 LLaMA、Qwen 等）并加载对应的分词器与因果语言模型。
import torch
from peft import PeftModel
# 来自 peft 库，专门用于加载和挂载参数高效微调（PEFT）权重（如 LoRA、QLoRA）。

mode_path = './LLM-Research/Meta-Llama-3___1-8B-Instruct'  # 基座模型本地路径
lora_path = './output/llama3_1_instruct_lora/checkpoint-699' # 训练输出的 LoRA 权重目录。这里改成你的 lora 输出对应 checkpoint 地址
...
# torch_dtype=torch.bfloat16：以 Brain Float 16 精度加载权重。相比 FP32 节省约 50% 显存，且比 FP16 更稳定（不易溢出），是现代 GPU（Ampere 架构及以上）推理的标准配置。
# .eval()：切换至推理模式，关闭 Dropout、BatchNorm 等训练期行为。
...
input_ids = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
# apply_chat_template：关键步骤。LLaMA-3.1 使用特殊的控制符（如 <|begin_of_text|>、<|start_header_id|> 等）区分角色。此方法将结构化消息转换为模型训练时看到的原始字符串。
# tokenize=False：不直接转成 Token ID，而是返回格式化后的字符串。方便调试或后续统一处理
# add_generation_prompt=True：自动追加 <|start_header_id|>assistant<|end_header_id|>，明确告诉模型“接下来该你回答了”。

# print(input_ids)
# 此时的input_ids是一个字符串

model_inputs = tokenizer([input_ids], return_tensors="pt").to('cuda')
# [input_ids]：将字符串包装成列表 [str]，显式保留 batch_size=1 维度。
# return_tensors="pt"：调用分词器将字符串切词，并转换为 PyTorch 张量。
# .to('cuda')：将张量移动到 GPU，与模型权重对齐。
generated_ids = model.generate(model_inputs.input_ids,max_new_tokens=512)
# 输入：提示词 Token 张量 [1, prompt_len]
# 输出：generated_ids 是 完整序列，包含 原始提示词 + 新生成词，形状为 [1, prompt_len + gen_len]。

generated_ids = [
    output_ids[len(input_ids):] for input_ids, output_ids in zip(model_inputs.input_ids, generated_ids)
]
response = tokenizer.batch_decode(generated_ids, skip_special_tokens=True)[0]
# batch_decode：将切片后的 Token ID 列表转回字符串。
# skip_special_tokens=True：自动过滤 LLaMA 特殊控制符（如 <|eot_id|>、<|start_header_id|> 等）。
# [0]：提取 batch 中第 0 个样本的文本。
